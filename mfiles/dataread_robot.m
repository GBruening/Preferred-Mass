function [T] = dataread_robot(subjid,filedir)

%DATAREAD  Opens and converts experiment datafiles
...
%         if i ~= trialno(i)
%             fprintf('Error: Trial Number mismatch for loop %d, trial %d',i,trialno(i));
%         else
...
for i=istart_tcl:length(tclindex)
    cd(filedir)
    [fid,message] = fopen(files{tclindex(i)});
    ...
    if fid > 0
        ...
        tcldata = textscan(fid,'%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%s%s%s%s%s');
        T.trialorder.types(i_tcl,1) = tcldata{1}(end,1);
        T.trialorder.rungs(i_tcl,1) = tcldata{2}(end,1);
        T.trialchoices(i_tcl,1) = tcldata{end}(end,1);
...
TrialList_cell = textscan(fid,'%f%f%f%f%f%f%f%f%f%s%s%f%f%f%f%f%f%f%f%f%f%f%f%f%f');
fclose(fid);
