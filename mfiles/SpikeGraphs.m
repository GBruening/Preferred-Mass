...
%             condition=1;
        ...
        firstavg(delcount)=mean(Data{c,subj}.avevel.all(1:5));%-Data{c-1,subj}.avevel.all(1:5));
        lastavg(delcount)=mean(Data{c,subj}.avevel.all(21:200))-mean(Data{c-1,subj}.avevel.all(21:200));
        
%         plot(delta_condition,firstavg(delcount),'o','Color',ColorSet(3,:),'MarkerFaceColor',ColorSet(3,:),'LineWidth',1);
%         plot(delta_condition,lastavg(delcount),'o','Color',ColorSet(15,:),'MarkerFaceColor',ColorSet(15,:),'LineWidth',1);
%         
%         plot(Data{c,subj}.avevel.all,'Color',ColorSet(c*3,:),'MarkerFaceColor',ColorSet(c*3,:),'LineWidth',1);
        
        trialstring={'Trial 1:5' 'Trial 6:10' 'Trial 11:15' 'Trial 16:20' 'Average'};
        
        Color(1)=plot(delta_condition,mean(Data{c,subj}.avevel.all(1:5))-mean(Data{c-1,subj}.avevel.all(1:5)),'o','Color',ColorSet(3,:),'MarkerFaceColor',ColorSet(3,:),'LineWidth',1);
        Color(2)=plot(delta_condition,mean(Data{c,subj}.avevel.all(6:10))-mean(Data{c-1,subj}.avevel.all(6:10)),'o','Color',ColorSet(6,:),'MarkerFaceColor',ColorSet(6,:),'LineWidth',1);
        Color(3)=plot(delta_condition,mean(Data{c,subj}.avevel.all(11:15))-mean(Data{c-1,subj}.avevel.all(11:15)),'o','Color',ColorSet(9,:),'MarkerFaceColor',ColorSet(9,:),'LineWidth',1);
        Color(4)=plot(delta_condition,mean(Data{c,subj}.avevel.all(16:20))-mean(Data{c-1,subj}.avevel.all(16:20)),'o','Color',ColorSet(12,:),'MarkerFaceColor',ColorSet(11,:),'LineWidth',1);
        ...
        One(loopnumber)=mean(Data{c,subj}.avevel.all(1:5))-mean(Data{c-1,subj}.avevel.all(1:5));
        Two(loopnumber)=mean(Data{c,subj}.avevel.all(6:10))-mean(Data{c-1,subj}.avevel.all(6:10));
        Three(loopnumber)=mean(Data{c,subj}.avevel.all(11:15))-mean(Data{c-1,subj}.avevel.all(11:15));
        Four(loopnumber)=mean(Data{c,subj}.avevel.all(16:20))-mean(Data{c-1,subj}.avevel.all(16:20));
...
%    hold off
        ...
        xlabel('Delta Condition');ylabel('Change in Avg Speed m/s');
