...
%             P(:,i) = (P(:,i)*-1)+.1;
        ...
        k=find(P(:,i)>.9*max(P(:,i)),1,'first');%MT.idxtarget(i);
        while P(k+1,i) > P(k,i)
            if k+1<length(P(:,i))
                k=k+1;
            else
        ...
        [a,b] = findpeaks(P(:,i));
        for k=length(a):-1:1
            if a(k)>.9*P(MT.idxmoveback(i),i)
                MT.idxmoveback(i) = b(k);
                break
            end
        end
     ...
     clear V_diff j
        ...
        j = MT.robotstates.wait4mvt(i)+...
            find(Data.TanV(MT.robotstates.wait4mvt(i):MT.idxmoveback(i),i)>.4*max(Data.TanV(MT.robotstates.wait4mvt(i):MT.idxmoveback(i),i)),1,'first');
        react1_found = 0;
        react3_found = 0;
        while  j>=12 %&& j>=MT.robotstates.wait4mvt(i)+1 %
            j=j-1;
            if (j <= 11 || std(Data.TanV(j-10:j,i))<2e-3 || sum(V_diff(j:j+10,i)>0)<4) && Data.TanV(j,i) < .2*max(Data.TanV(1:MT.idxmoveback(i),i)) && react1_found == 0
                MT.idxonset(i)=j;
                react1_found = 1;
            end
            if (j >= 11 || std(Data.TanV(j-10:j,i))<2e-3 || sum(V_diff(j:j+10,i)>0)<4)&& Data.TanV(j,i) < .2*max(Data.TanV(1:MT.idxmoveback(i),i)) && react3_found == 0 && j >= MT.robotstates.wait4mvt(i)
                MT.idxonset3(i)=j;
                react3_found = 1;
                j9 = j9+1;
...
%         MT.idxonset3(i)=lastlowAccVel;
%         MT.idxonset(i)=lastlowAccVel;  
...
%         end
        ...
        vsign(i)=1;
        [MT.peakvy(i),b] = nanmax(V(MT.robotstates.wait4mvt(i):MT.idxendpt(i),i),[],1);
        ...
        a=find(Data.TanV(MT.idxonset(i):MT.idxendpt(i),i)>0.01,1,'first');
        if ~isempty(a)
            MT.idxvthresh_onset(i) = a+MT.idxonset(i);
        else
            MT.idxvthresh_onset(i) = find(Data.TanV(:,i)>.2*max(Data.TanV(20:end,i)),1,'first')+20;
        end
