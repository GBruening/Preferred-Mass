...
for c=4:-1:1
    for t=1:4
        for d=2:2
            ...
            if max(eff_mass{c,t})>max1
                max1 = max(eff_mass{c,t});
            end
            if min(eff_mass{c,t})<min1
                min1 = min(eff_mass{c,t});
            end
%             angle = atan2(Data{c,t}.targetposition(1),Data{c,t}.targetposition(2)-.4);
%             max(eff_mass{c,t})-min(eff_mass{c,t});
%             a(c) = polar(angle,max(eff_mass{c,t}),'x');%-min(eff_mass{c,t})
...
for c=4:-1:1
    for t=1:4
        for d=2:2
            angle = atan2(Data{c,t}.targetposition(1)-ro(1),Data{c,t}.targetposition(2)-ro(2));
            max(eff_mass{c,t})-min(eff_mass{c,t});
            a(c) = polarplot(angle,max(eff_mass{c,t}),'x');%-min(eff_mass{c,t})
            set(a(c),'markerfacecolor',colorset(c,:),'color',colorset(c,:),'linewidth',5);
            hold on;
        end
    end
...
for c=4:-1:1
    for t=1:4
        for d=2:2
            angle = atan2(Data{c,t}.targetposition(1)-ro(1),Data{c,t}.targetposition(2)-ro(2));
%             max(eff_mass{c,t})-min(eff_mass{c,t});
            a(c) = polarplot(angle,min(eff_mass{c,t}),'x');
            set(a(c),'markerfacecolor',colorset(c,:),'color',colorset(c,:),'linewidth',5);
            hold on;
        end
    end
...
for c=4:-1:1
    for t=1:4
        for d=2:2
            angle = atan2(Data{c,t}.targetposition(1)-ro(1),Data{c,t}.targetposition(2)-ro(2));
            max(eff_mass{c,t})-min(eff_mass{c,t});
            a(c) = polarplot(angle,max(eff_mass{c,t})/min(eff_mass{c,t}),'x');
            set(a(c),'markerfacecolor',colorset(c,:),'color',colorset(c,:),'linewidth',5);
            hold on;
        end
    end
...
for c=4:-1:1
    for t=1:4
        for d=2:2
            angle = atan2(Data{c,t}.targetposition(1)-ro(1),Data{c,t}.targetposition(2)-ro(2));
            mean(eff_mass{c,t});
            a(c) = polarplot(angle,mean(eff_mass{c,t}),'x');
            set(a(c),'markerfacecolor',colorset(c,:),'color',colorset(c,:),'linewidth',5);
            hold on;
        end
    end
...
for k = 1:16
    ...
    s=3;
    for d = 2:2
        plot(eff_mass{c,t},'Color',ColorSet(c,:),'linewidth',3);
        text(length(eff_mass{c,t}),eff_mass{c,t}(end),...
            sprintf('%.2f',eff_mass{c,t}(end)));
    end
%     ylim([min(eff_mass{c,t})*.99,max(eff_mass{c,t})*1.01]);
    x=gca;
    text(10,x.YTick(3),sprintf('%.2f',eff_mass{c,t}(1)));
...
hold on;
    for c = 1:4
    plotted = plot(eff_mass{c,t},'Color',colorset(c,:));
    end
end
...
for s = 1:6
    summed = 0;
    for t = 1:4
        summed = summed + max(eff_mass{c,t});
    end
...
for s = 1:6
    summed = 0;
    for t = 1:4
        summed = summed + min(eff_mass{c,t});
    end
...
for s = 1:6
    summed = 0;
    for t = 1:4
        summed = summed + sum(eff_mass{c,t})*.005/vars{c,t}.speeds(s);
    end
...
function [y_pred] = func_fit2(a,x,vars,y)
    ...
    s1=[vars{1,1}.speeds;vars{2,1}.speeds];
    s2=[vars{3,1}.speeds;vars{4,1}.speeds];  
    s = [s1;s2];
    y_pred = func(a,x,s);
