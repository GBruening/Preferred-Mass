...
%    nTrials_speed = 200; % look at last X trials only; for all, popts.totaltrials
    for c=1:nc
        ...
        for i=popts.totaltrials-nTrials_speed+1:popts.totaltrials
            ...
            Data{c,subj}.reaction_vthres(tr_count)=MT{c,subj}.idxonset(i)-MT{c,subj}.robotstates.wait4mvt(i); %This is vthres anymore, its the new method
            ...
            %Data{c,subj}.miss_angle(tr_count) = atan2(norm(cross(targetvector,missvector)),dot(targetvector,missvector));
            ...
            Data{c,subj}.corr_angle(tr_count) = atan2d(norm(cross(initvector,missvector)),dot(initvector,missvector));
            Data{c,subj}.inittotar_angle(tr_count) = atan2d(norm(cross(initvector,targetvector)),dot(initvector,targetvector));
...
%             RobotState{c,subj}.eight=mean(Data{c,subj}.stateframes(8,:));
        ...
        PathL.all(c,subj)=mean(Data{c,subj}.pathlength.all);
         ...
         fprintf('Subject %d %s Condition %s Processed\n',subj,subjid,condition);
